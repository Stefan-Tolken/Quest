...
export default function Quests() {
  ...
  const loadQuestProgress = useCallback(async (questId: string) => {
    ...
    try {
      ...
      // Check if this quest is already completed by the user
      const isCompleted = isQuestCompleted(questId);
      if (isCompleted) {
        // Quest is completed, no need to load progress
        setProgress(null);
        return;
      }
  ...
  }, [handleError, isQuestCompleted]);

  // Load quest progress when active quest changes
  useEffect(() => {
  ...
  }, [activeQuest, loadQuestProgress]);

  // Check for quest completion when progress changes
  useEffect(() => {
    ...
    if (
      ...
      if (collectedCount >= totalArtefacts) {
        ...
        // Clear local progress state since it will be cleaned up
        setProgress(null);
      }
    }
